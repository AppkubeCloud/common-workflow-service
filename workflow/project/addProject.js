...
exports.handler = middy(async (event, context) => {
	...
	const {  name ,description, department, start_date, end_date, image_url } = JSON.parse(event.body);
	const client = await connectToDatabase();
		const isDuplicate = await client.query(`SELECT COUNT(*) FROM projects_table WHERE LOWER(project->>'name') = LOWER($1)`, [name]);
